...
Todo
─────────────────────────────────────────────────────────────────────
- [ ] UX: Interactieve kaart (klik bestemming → bereken prop + teken hop-lijn)
- [ ] UX: Thema-configuratie (ondersteuning voor meerdere donkere thema's/high-contrast)
- [ ] Data: Uitbreiden ionosonde-netwerk naar wereldwijde stations
- [ ] Data: Voorspellend model toevoegen op basis van 48u trendanalyse
- [ ] Data: WSPR filters implementeren (SNR drempel, band-selectie)
- [ ] Stab: Implementeren van "Offline" indicator bij netwerkuitval
- [ ] Stab: Validatie toevoegen aan HAMIOS.ini loading process
- [ ] CAT: Retry-mechanisme voor seriële poorten implementeren en _CAT_DISABLED op False zetten
- [ ] Vis: Heatmap implementeren voor historische DX-spot activiteit
─────────────────────────────────────────────────────────────────────
Change Log (2.3)
─────────────────────────────────────────────────────────────────────
...
class HAMIOSApp:
    def __init__(self, root: tk.Tk):
        ...
        # Apply initial theme before build_ui
        self._apply_theme()
    ...
    def _apply_theme(self):
        """Doorloop alle widgets en update kleuren op basis van ThemeManager."""
        tm = self.theme_manager

        # Root window
        self.root.configure(bg=tm.get_color("BG_ROOT"))

        def walk(widget):
            try:
                # Frames & Containers
                if isinstance(widget, (tk.Frame, tk.Toplevel, tk.LabelFrame)):
                    cur_bg = widget.cget("bg")
                    if cur_bg == BG_PANEL: widget.configure(bg=tm.get_color("BG_PANEL"))
                    elif cur_bg == BG_ROOT: widget.configure(bg=tm.get_color("BG_ROOT"))
                    elif cur_bg == BG_SURFACE: widget.configure(bg=tm.get_color("BG_SURFACE"))
                    elif cur_bg == BG_HOVER: widget.configure(bg=tm.get_color("BG_HOVER"))
                    elif cur_bg == BORDER: widget.configure(bg=tm.get_color("BORDER"))

                # Labels
                if isinstance(widget, tk.Label):
                    cur_fg = widget.cget("fg")
                    if cur_fg == TEXT_H1: widget.configure(fg=tm.get_color("TEXT_H1"))
                    elif cur_fg == TEXT_BODY: widget.configure(fg=tm.get_color("TEXT_BODY"))
                    elif cur_fg == TEXT_DIM: widget.configure(fg=tm.get_color("TEXT_DIM"))
                    elif cur_fg == ACCENT: widget.configure(fg=tm.get_color("ACCENT"))
                ...
                if isinstance(widget, tk.Button):
                    cur_bg = widget.cget("bg")
                    if cur_bg == BG_SURFACE: widget.configure(bg=tm.get_color("BG_SURFACE"))
                    # We don't theme a specific color buttons (Exit, CAT) unless they use BG_SURFACE

                # Entries
                if isinstance(widget, tk.Entry):
                    widget.configure(bg=tm.get_color("BG_SURFACE"),
                                    fg=tm.get_color("TEXT_H1"),
                                    insertbackground=tm.get_color("TEXT_H1"))

                # Checkbuttons / Radiobuttons
                if isinstance(widget, (tk.Checkbutton, tk.Radiobutton)):
                    widget.configure(bg=tm.get_color("BG_PANEL"),
                                    fg=tm.get_color("TEXT_BODY"),
                                    selectcolor=tm.get_color("BG_SURFACE"),
                                    activebackground=tm.get_color("BG_PANEL"),
                                    activeforeground=tm.get_color("TEXT_H1"))
