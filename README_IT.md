# 📡 HAMIOS v4.0.1

**Monitor di propagazione HF e DX per Windows**

> v4.0.1 — Pannelli trascinabili e ridimensionabili · Finestra di dialogo Impostazioni unificata · Ottimizzazioni delle prestazioni · Grafico storico solare

*Conceived by Frank van Dijke · Developed with Claude AI*

HAMIOS offre ai radioamatori dati in tempo reale sulla propagazione HF, attività solare e opportunità DX — in un'interfaccia grafica moderna e scura.

---

## ✨ Funzionalità

### 🌍 Carta mondiale interattiva *(centrata nella finestra)*
- Carta equirettangolare NASA, scaricata automaticamente al primo avvio
- **Altezza fissa (380 px)** con rendering 2:1 corretto — nessuna distorsione orizzontale
- **Terminatore giorno/notte** con zona di transizione morbida
- **Linea grigia** (banda ambra, ~1000 km di larghezza)
- **Posizione sole e luna** (calcolo geocentrico)
- **Marcatore QTH** (mirino blu) — lat/lon configurabile nell'intestazione
- **Percorso cerchio massimo**: clic sulla mappa → distanza + rotta + **qualità banda per quel percorso**
- **Overlay regioni UIT** R1/R2/R3 (ufficiale UIT RR Art. 5)
- **Griglia locatori Maidenhead** (20° × 10°, etichette AA–RR)
- **Livello prefissi nominativi** (~110 entità DXCC)
- **Zoom/panoramica**: rotella 1×–8×, clic+trascina per panoramica, clic destro per ripristino
- **Overlay raggruppati** sotto la mappa:
  - *Display:* Sun · Moon · Gray line · Aurora · Sat
  - *Data:* WSPR · Spots · CS · Locator

### 🛰️ Propagation Path Map
- **Band quality** calculated at the midpoint of the great-circle path
- Day/night correction at the midpoint
- Number of hops estimated (≈ distance / 3500 km)
- Top-5 open bands: `↳  8,847 km / 3 hops / day  ▸  20m 87%  ·  17m 72%  ·  15m 54%  (MUF 24.1 MHz)`
- **Great-circle line colour follows the best band**

### 🌌 Aurora Ring Overlay
- Magnetic aurora oval based on K-index (Feldstein/Holzworth, IGRF-2025)
- **Colour by K-index**: green (K < 3) · yellow (K 3–5) · red (K ≥ 6)

### 🖥️ Sistema a pannelli *(v4.0.1)*
- **11 pannelli liberamente trascinabili e ridimensionabili** su una tela desktop libera
- Ogni pannello ha un bordo ambra da 1px, una barra del titolo con pulsante ✕ di chiusura e una maniglia ◢ di ridimensionamento
- **Aggancio alla griglia** (2px) al rilascio del trascinamento/ridimensionamento per un allineamento preciso
- Pannelli: Affidabilità bande HF · Carta mondiale · Solare/Ionosfera · Avvisi · Pianificazione bande · Storico bande · Kp 48h · Bz 24h · Raggi X 24h · Spot DX · Consigli di propagazione
- **⚙ Finestra di dialogo Impostazioni** contiene tutta la configurazione: QTH · Lingua · Tema · Suggerimenti · Ticker · Ora legale · Visibilità pannelli · Gestione layout
- **Preimpostazioni layout**: salvataggio/caricamento/sovrascrittura/eliminazione di profili denominati in `hamios_layouts.json`
- **Salva come predefinito**: salva il layout corrente come layout di avvio

### 🛰 Tracciamento satelliti *(pulsante 🛰 Sat nell'intestazione)*
- Dati TLE scaricati da **Celestrak** (Amateur / ISS / Weather / CubeSat)
- **Filtro per categoria**: Tutti · Amateur · ISS · Weather · CubeSat
- Per satellite: attivare/disattivare **punto di posizione** (●), **percorso orbitale** (~) e **footprint** sulla mappa
- **Il footprint diventa verde** quando il QTH è nella zona di copertura del satellite
- **Pannello notifiche** mostra i satelliti attualmente sopra il QTH con angolo di elevazione
- Orari del percorso in ora locale (CEST / CET)
- Pulsante **↻ TLE** per aggiornamento manuale degli elementi orbitali
- Memorizzato nella cache in `hamios_tle.json`

### 🕵 Stazioni spia / numeri *(pulsante 🕵 Spy nell'intestazione)*
- Tabella scorrevole di 24 note stazioni a numeri e stazioni radio spia
- Colonne: stato (● attivo / storico), nome, paese, frequenze, modo
- **Colonne ordinabili** — clic su un'intestazione per ordinare; clic di nuovo per invertire
- **Filtro** per stato attivo/inattivo e ricerca a testo libero (nome, paese, frequenza)
- **Passare il mouse** su una riga per vedere la descrizione completa + programma di trasmissione
- File di dati: `hamios_spy_stations.json` (modificabile)

### 🔵 WSPR / PSKReporter Spots on Map
- Live propagation paths from **wspr.rocks** (WSPR) and **pskreporter.info** (FT8/FT4)
- Connection lines: **colour = band**, **thickness = SNR**

### 📍 DX Spot Markers on Map
- Active DX cluster spots: dot at DX location + line to spotter, colour-coded by band
- Click a dot to show callsign, frequency and comment

### 🌞 Dati solari e ionosferici

| Parameter | Source | Description |
|-----------|--------|-------------|
| SFI | hamqsl.com | Solar Flux Index (10.7 cm) |
| SSN | hamqsl.com | Sunspot Number |
| A-index | hamqsl.com | Daily geomagnetic activity |
| K-index | hamqsl.com | 3-hour geomagnetic activity (0–9) |
| X-ray | hamqsl.com | GOES X-ray flux (A/B/C/M/X class) |
| MUF | internal model | Maximum Usable Frequency |
| LUF | internal model | Lowest Usable Frequency |
| Solar wind speed | NOAA SWPC | km/s via DSCOVR/ACE |
| Bz | NOAA SWPC | Interplanetary magnetic field Z-component (nT) |
| foF2 | GIRO/LGDC | Measured F2-layer critical frequency (ionosonde) |

### 📊 Bz 24-hour Chart *(dedicated panel)*
- **Blue** = positive Bz (favourable) · **Red** = negative Bz (geo-effective)
- Y-labels −20 / 0 / +20 nT · scales automatically with the panel

### 📶 HF Band Reliability *(redesigned in v3.0)*
- Internal MUF/LUF model based on SFI, SSN, K-index and QTH latitude
- SNR budget corrected for **mode**, **power** and **antenna**
- **Gradient bars in band-specific colour** (light → dark) with gloss line
- Band name **bold and in band colour** — every band has its own identity
- Columns: start MHz · modes · FT8 frequency

### 🔔 Notifications & Alerts

| Notification | Setting | Description |
|-------------|---------|-------------|
| ⚠️ K-index storm | Checkbox + threshold (1–9) | Alert when K rises above threshold |
| 📡 Band open | Checkbox + threshold (10–90 %) | Alert when band opens |
| 🛰 Satellite sopra | Sempre attivo | Mostra quali satelliti selezionati sono sopra il QTH con elevazione |

Thresholds are saved to `HAMIOS.ini`.

### 🔌 CAT Interface *(temporarily disabled)*
> The CAT interface is temporarily disabled pending further development. Code is intact and will be completed in a future release.

### 🕐 Band Opening Schedule
- 24-hour heatmap for all 11 HF bands
- Time axis in local time (CET/CEST), current time highlighted
- Hover tooltip: band · hour · quality · modes · FT8 frequency

### 📈 Band History
- Time graph of all HF bands (%) — 90 days kept in CSV
- Time range: Hours · Days · Weeks · Months
- Clickable legend to toggle individual bands

### 📡 Live DX Spots *(full-height right column)*
- JSON feed from dxwatch.com, refreshed every 5 minutes
- Columns: UTC · Band · DX · MHz · Spotter · Comment
- **Own-continent filter** — show only spots from your own continent
- **Heatmap mode**: band × UTC-hour activity pattern (24h buffer)
- Spots also shown as markers on the world map (toggleable)

### 💡 Propagation Analysis & Advice
12 analysis cards in 3 columns × 4 rows.

| # | Card | Contents |
|---|------|----------|
| 1 | 📡 Best bands | Top-5 open bands with quality percentage |
| 2 | ✅/⚠️ Geo-storm | K + A-index, route advice, severity |
| 3 | ☀️ Solar activity | SFI + SSN, solar cycle phase |
| 4 | 💨 Solar wind | Speed + Bz impact |
| 5 | ☢️ X-flare/SWF | X-ray class, recovery estimate |
| 6 | 📶 Ionosphere | MUF, LUF, F2 layer |
| 7 | 🌅 Day/night | Gray-line time windows |
| 8 | 🔧 Mode advice | FT8 vs SSB/CW based on SNR budget |
| 9 | 🧲 Absorption | Auroral absorption polar routes |
| 10 | 📊 Overall score | Composite propagation rating |

### 🌐 Multilingual
**14 languages** via external language pack system (`langs/*.json`).

| Code | Language | Code | Language |
|------|----------|------|----------|
| EN | English *(built-in)* | NO | Norsk |
| NL | Nederlands | PL | Polski |
| DE | Deutsch | SV | Svenska |
| FR | Français | DA | Dansk |
| IT | Italiano | FI | Suomi |
| ES | Español | PT | Português |
| JA | 日本語 | RU | Русский |

### ⚙️ Altro
- **Temi dinamici**: Midnight · DeepOcean · HighContrast
- **Barra di sistema**: riduci a icona nella barra, notifiche
- **Suggerimenti** con spiegazione per ogni parametro solare
- **Aggiornamento automatico**: Off / 30 s / 1 min / 5 min / 10 min / 30 min / 1 ora
- **Ticker scorrevole** con suggerimenti di propagazione attuali
- **Sistema a pannelli trascinabili**: disponi e ridimensiona liberamente tutti i pannelli
- **Preimpostazioni layout**: salva e passa tra disposizioni di pannelli personalizzate
- Tutte le impostazioni salvate in `HAMIOS.ini`

---

## 🖥️ Layout (v4.0.1) — Pannelli flottanti liberi

```
┌─────────────────────────────────────────────────────────────────────────┐
│  ⚙ Impostazioni  (QTH · Lingua · Tema · Suggerimenti · Ticker · Ora legale · Pannelli · Layout) │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌──────────────┐  ┌──────────────────────────────┐  ┌──────────────┐  │
│  │Solar/Ionosfer│  │       Carta mondiale      ✕  │  │Bande HF  ✕  │  │
│  │     ✕    ◢  │  │   zoom/pan · overlay      ◢  │  │Affidabilità◢│  │
│  └──────────────┘  └──────────────────────────────┘  └──────────────┘  │
│                                                                         │
│  ┌─────────────┐  ┌─────────────┐  ┌──────────┐  ┌──────────────────┐  │
│  │Piano. bande │  │Storico bande│  │  Kp 48h  │  │  Spot DX     ✕  │  │
│  │    ✕    ◢  │  │    ✕    ◢  │  │  ✕    ◢  │  │             ◢  │  │
│  └─────────────┘  └─────────────┘  └──────────┘  └──────────────────┘  │
│                                                                         │
│  ┌──────────┐  ┌──────────┐  ┌──────────────────────────────────────┐  │
│  │  Bz 24h  │  │RaggX 24h │  │  Consigli di propagazione       ✕  │  │
│  │  ✕    ◢  │  │  ✕    ◢  │  │                              ◢  │  │
│  └──────────┘  └──────────┘  └──────────────────────────────────────┘  │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  Avvisi  ✕  ◢                                                   │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  Tutti i pannelli: bordo ambra · ✕ chiudi · ◢ ridimensiona · griglia(2px) │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 🖥️ Installazione

### Passo 1 — Requisiti Python

```bash
pip install pillow
```

Optional for system tray notifications:

```bash
pip install pystray
```

### Passo 2 — Installare i pacchetti lingua

HAMIOS ships with English as the built-in language. All other languages are in separate `.json` files.

**Download:**
Get the language files from the [latest release](https://github.com/fvdijke/HAMIOS/releases) — download `langs.zip`.

**Directory structure:**
```
HAMIOS.exe  (or HAMIOS.py)
langs\
  lang_nl.json    ← Nederlands
  lang_de.json    ← Deutsch
  lang_fr.json    ← Français
  lang_it.json    ← Italiano
  lang_es.json    ← Español
  lang_no.json    ← Norsk
  lang_pl.json    ← Polski
  lang_sv.json    ← Svenska
  lang_da.json    ← Dansk
  lang_fi.json    ← Suomi
  lang_pt.json    ← Português
  lang_ja.json    ← 日本語
  lang_ru.json    ← Русский
```

> The `langs/` folder must be placed **next to** `HAMIOS.exe` or `HAMIOS.py`.

**Creating your own language pack:**
Copy an existing `.json` file, change `meta.code` and `meta.name`, then translate the `strings` and `solar_tips` sections. HAMIOS will detect the new pack automatically on startup.

### Passo 3 — Avviare

```bash
python HAMIOS.py
```

### EXE standalone (Windows) — consigliato

Download `HAMIOS.exe` and the `langs/` folder from the [latest release](https://github.com/fvdijke/HAMIOS/releases). No Python required.

Build from source:

```bash
pip install pyinstaller
pyinstaller HAMIOS.spec
# → dist\HAMIOS.exe
# Then copy langs\ next to the EXE
```

---

## 📁 File di dati

| File | Contenuto |
|------|-----------|
| `HAMIOS.ini` | Tutte le impostazioni utente (QTH, tema, lingua, soglie di allerta, …) |
| `hamios_layouts.json` | Preimpostazioni layout pannelli |
| `hamios_tle.json` | Elementi orbitali TLE in cache per il tracciamento satelliti |
| `hamios_spy_stations.json` | Database stazioni spia/numeri (modificabile) |
| `langs/*.json` | Pacchetti lingua |

---

## 🔭 Modello di propagazione

```
foF2 = 4.0 + (SFI − 70) × 0.065 + SSN × 0.012
MUF  = foF2 × latitude-factor × day/night-factor × 3.8
LUF  = (3.5 + K × 0.8) × auroral-factor / 10^(SNR/20)
```

**Band quality**: optimum around 55 % of the MUF/LUF window → 100 %.

---

## ⌨️ Interazione mouse

| Action | Effect |
|--------|--------|
| Scroll wheel on map | Zoom 1×–8× |
| Click + drag on map | Pan viewport |
| Left-click on map | Great-circle path + path propagation |
| Right-click on map | Reset zoom/pan · clear path |
| Hover on map / bands / chart | Detailed tooltip |
| Click on legend label | Toggle band in history chart |

---

## 💻 Requisiti di sistema

| | Minimum | Recommended |
|-|---------|------------|
| OS | Windows 10 | Windows 11 |
| Screen resolution | 1280 × 900 | 1920 × 1080 or wider |
| Python | 3.10 | 3.12+ |
| Internet | Required (data feeds) | — |

> v4.0.1 utilizza una tela di pannelli libera. Disponi i pannelli liberamente su qualsiasi dimensione dello schermo. Un display ampio (≥ 1920 px) offre lo spazio di lavoro più comodo.

---

## 🔧 Idee future

- Stabilise CAT interface (Yaesu/Kenwood/Icom)
- SDR integration
- Logging connection (ADIF/WSJTX)

---

## 📜 Licenza

Gratuito per uso personale e non commerciale in radioamateurismo.

---

