...
> v4.0.1 — ドラッグ可能なリサイズ対応パネル · 統合設定ダイアログ · パフォーマンス最適化 · 太陽活動履歴グラフ
...
### 🖥️ パネルシステム *(v4.0.1)*
- **11の自由にドラッグ可能でリサイズ可能なパネル**を自由なデスクトップキャンバスに配置
- 各パネルはアンバー色の1pxボーダー、✕閉じるボタン付きタイトルバー、◢リサイズハンドルを備える
- ドラッグ/リサイズ解放時に**スナップ・トゥ・グリッド**（2px）で整列
- パネル: HFバンド信頼性 · 世界地図 · 太陽/電離層 · アラート · バンドスケジュール · バンド履歴 · Kp 48h · Bz 24h · X線 24h · DXスポット · 伝搬アドバイス
- **⚙ 設定ダイアログ**にすべての設定: QTH · 言語 · テーマ · ツールチップ · ティッカー · 夏時間 · パネル表示 · レイアウト管理
- **レイアウトプリセット**: `hamios_layouts.json`に保存された名前付きプロファイルの保存/読込/上書き/削除
- **デフォルトとして保存**: 現在のレイアウトをスタートアップレイアウトとして保存
...
### ⚙️ その他
- **ダイナミックテーマ**: Midnight · DeepOcean · HighContrast
- **システムトレイ**: トレイに最小化、トレイ通知
- **ツールチップ**で各太陽パラメータを説明
- **自動更新**: オフ / 30秒 / 1分 / 5分 / 10分 / 30分 / 1時間
- **スクロールティッカー**で現在の伝搬ヒントを表示
- **ドラッグ可能なパネルシステム**: すべてのパネルを自由に配置・サイズ変更
- **レイアウトプリセット**: カスタムパネル配置を保存・切替
- すべての設定を`HAMIOS.ini`に保存

---

## 🖥️ レイアウト (v4.0.1) — 自由浮動パネル

```
┌─────────────────────────────────────────────────────────────────────────┐
│  ⚙ 設定  (QTH · 言語 · テーマ · ツールチップ · ティッカー · 夏時間 · パネル · レイアウト) │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌──────────────┐  ┌──────────────────────────────┐  ┌──────────────┐  │
│  │太陽/電離層   │  │       世界地図            ✕  │  │HFバンド  ✕  │  │
...
│                                                                         │
│  ┌─────────────┐  ┌─────────────┐  ┌──────────┐  ┌──────────────────┐  │
│  │バンドスケジュール│  │バンド履歴  │  │  Kp 48h  │  │DXスポット    ✕  │  │
│  │    ✕    ◢  │  │    ✕    ◢  │  │  ✕    ◢  │  │             ◢  │  │
│  └─────────────┘  └─────────────┘  └──────────┘  └──────────────────┘  │
│                                                                         │
│  ┌──────────┐  ┌──────────┐  ┌──────────────────────────────────────┐  │
│  │  Bz 24h  │  │X線 24h   │  │  伝搬アドバイス                  ✕  │  │
│  │  ✕    ◢  │  │  ✕    ◢  │  │                               ◢  │  │
│  └──────────┘  └──────────┘  └──────────────────────────────────────┘  │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
...
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  全パネル: アンバーボーダー · ✕ 閉じる · ◢ リサイズ · グリッド吸着(2px)│
└─────────────────────────────────────────────────────────────────────────┘
```
...
| ファイル | 内容 |
|--------|------|
| `HAMIOS.ini` | すべてのユーザー設定（QTH、テーマ、言語、アラートしきい値など） |
| `hamios_layouts.json` | パネルレイアウトプリセット |
| `hamios_tle.json` | 衛星追跡用キャッシュTLE軌道要素 |
| `hamios_spy_stations.json` | スパイ局/ナンバーズ局データベース（編集可能） |
| `langs/*.json` | 言語パック |
...
> v4.0.1は自由なパネルキャンバスを使用します。あらゆる画面サイズでパネルを自由に配置できます。ワイドディスプレイ（≥ 1920 px）が最も快適な作業空間を提供します。
